    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Secret()
        {
            var serverResponse = await AccessTokenRefreshWrapper(
                () => SecuredGetRequest("https://localhost:44306/secret/index"));

            var apiResponse = await AccessTokenRefreshWrapper(
                () => SecuredGetRequest("https://localhost:44322/secret/index"));
        ...
        private async Task RefreshAccessToken()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost:44306/oauth/token")
            {
